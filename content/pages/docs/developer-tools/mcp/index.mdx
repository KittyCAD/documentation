...
4. Have the assistant save `main.kcl` and any required project files in a dedicated CAD project folder using the client's authorized file-editing tools.
5. Have it execute the saved project through Zoo MCP using `kcl_path` and inspect the geometry.
6. Open `main.kcl` in Zoo Design Studio and keep the KCL project as the editable deliverable. Request STEP or STL exports when needed.
...
Zoo MCP does not have a separate fee, but tools that use Zoo's hosted services count as API usage outside Zoo Design Studio. See the [API pricing FAQ](/docs/faq#faq-api-pricing) for current billing details.
