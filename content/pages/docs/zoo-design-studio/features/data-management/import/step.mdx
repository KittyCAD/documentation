...
With the STEP file in Project Files, right-click it and choose **Import in current file**.
Complete the Import command to add the import statement to the active KCL file. Zoo Design Studio
fills in the file path for you, so you do not need to type the filename manually.

Selecting a STEP file or expanding a folder does not start an import.
