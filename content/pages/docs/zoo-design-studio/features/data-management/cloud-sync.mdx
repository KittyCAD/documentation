...
- **Projects in Local Projects do not sync.** They stay in the folder you chose on that computer
  unless you move them to Personal Cloud or publish them to Aquarium. This is also true for projects
  in any other local library. Publishing automatically moves a local project to Personal Cloud after
...
The project library determines where the editable project is saved, Cloud Sync keeps a Personal
Cloud project current across devices, and publishing submits a version of that project to Aquarium.
Personal Cloud projects are not public unless you publish them.
...
beside Local Projects with a cloud icon. Local Projects has a folder icon and shows a normal path on
your computer. You can rename it or add other local libraries. In the browser, Personal Cloud is the
only project library.
...
projects in Local Projects or another local library have been uploaded.
...
your Zoo account. Projects in `Local Projects` or another local library are local-only, even while
Cloud Sync is enabled. Move a local project to Personal Cloud when you want to start syncing it
directly. Publishing a local project to Aquarium also moves it to Personal Cloud and starts syncing
it.
...
### Are Personal Cloud Projects Private?

Yes. Today, unpublished Personal Cloud projects remain private in your Zoo account on both Free and
paid plans. Changing or canceling a subscription does not publish or remove them; after a downgrade,
the projects remain private in the same account on the Free plan.

Zoo may introduce a semi-public storage state for Free-plan projects in the future. That behavior,
including any associated data-use or model-training policy, is not active today. Zoo will update
its website and documentation before enabling a different visibility or data-use policy.
...
### Does a Project in Local Projects Still Use Zoo's Cloud Modeling?

Yes. Project storage and modeling computation are separate. A project can remain local in a
local library while Zoo Design Studio uses Zoo's cloud services to execute KCL, render the
...
To start syncing a project from `Local Projects` or another local library, move it to `Personal
Cloud`:
...
{/* VIDEO PLACEHOLDER: Moving a project from Local Projects to Personal Cloud and watching the sync
...
To make a synced project local-only, move it from `Personal Cloud` to `Local Projects` or another
local library:

1. Select `Move to library` from the Command Bar, the project menu in the top bar, or the project
   card context menu on Home.
2. Choose the synced project.
3. Choose `Local Projects` or another local library as the destination.
4. Confirm the move.
...
{/* VIDEO PLACEHOLDER: Moving a Personal Cloud project into Local Projects to make it local-only. */}
