...
To save a PNG of the current viewport instead of exporting model geometry, use
[Capture Screenshot](/docs/zoo-design-studio/features/data-management/export/capture-screenshot).

## Supported Export Formats

{/* Keep this table in sync with modeling-app's Export command options. */}

These are the formats currently exposed by Zoo Design Studio's Export command. The Engine API may
support additional conversion formats that are not available in the Design Studio UI.
...
- Preserves precise solid geometry, including multi-body models
- Best for CAD system interchange (SolidWorks, CATIA, NX, Creo, Inventor, Fusion 360)
- Exports each solid as a separate STEP part
...
To create an STL and open it directly in a supported desktop slicing application, see
[Export to Slicer](/docs/zoo-design-studio/features/data-management/export/export-to-slicer).
...
[File Format API](/docs/developer-tools/api/file) or the
[`zoo file convert`](/docs/cli/manual/zoo_file_convert) CLI command.

## Export Command

The Export command exports the current model, including parts and assemblies. For step-by-step
instructions, see [Export a part](/docs/zoo-design-studio/features/data-management/export/export-brep#export-a-part)
or [Export an assembly](/docs/zoo-design-studio/features/data-management/export/export-brep#export-an-assembly).
...
Export includes the geometry produced by the active KCL file. To export a complete assembly from
a multi-file project, open the file that builds it, usually `main.kcl`, and export once.

- **STEP format:** Each solid becomes a separate part. Most CAD applications interpret these parts
  as an assembly; repeated parts are not exported as linked instances.
...
See [Export an assembly](/docs/zoo-design-studio/features/data-management/export/export-brep#export-an-assembly)
for the full workflow and what transfers to another CAD system.
