...
Our LSP autcompletion was built a long time ago, back when KCL functions used _positional_ arguments, not _named_ arguments. Back then, a function like `circle` was called like `circle([0, 0], 4)` instead of `circle(center = [0, 0], radius = 4)`. So our developer tools, like the LSP, were developed without considering argument labels. We're slowly improving our developer tools though, so keep checking back!
