...
Note that the mesh example here is somewhat forced; it's trying to create a surface while the others are simply defining boundaries—but if you imagine this example as a sphere in 3D instead of a circle 2D, you can see how both B-rep and implicit representations could be used to define the sphere's surface analytically, while the mesh will always be a discrete approximation.
...
Mechanical components almost always have strict requirements—precise tolerances, standardized fits, and clearly defined geometric relationships; exactly what parametric B-rep modeling is designed for. By explicitly encoding both geometric precision and design intent, parametric B-rep models enable engineers to reliably ideate, develop, and communicate design details. Their explicit topological and parametric precision ensures consistency and accuracy throughout the design and manufacturing cycle, supporting everything from generating production-ready drawings to providing clean, well-defined input for downstream simulation... even if most simulation techniques will involve generating a mesh of their own!
...
When I think about typical design and development processes, it strikes me that engineering data is hard to work with. It tends to be wholly unstructured; fragmented and siloed, and in many different formats. You may have CAD data, spreadseets, supplier datasheets, model files, simulation results, and test data, all held in different formats—some of them proprietary—and all stored in different places. Description of intent is often poorly captured, if at all. It may perhaps be found, indirectly, in reports and meeting notes and e-mail chains, but it is rarely held in a form that can be easily consumed by a model wishing to infer the rationale behind a given decision.

Even if one finds a readily accessible source of that information, there remains a clear disconnect between design intent and the geometry that has resulted from it. A CAD file may describe the shape of an object or a feature very accurately, but it has no means by which it can tell you you _why_ it's shaped that way. Did your colleague up the OD on that shaft because something failed? Perhaps it's easier to make like this, or maybe they saw some evidence of resonance and wanted to move the natural frequency. 
