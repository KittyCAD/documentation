...
The reason why a GUI does make sense for us, is we are not giving up on code, in fact, we’re doing the opposite. We're using a GUI to enhance the code-authoring experience. We're combining the two.
...
Since users are still in control of the code at every step, any changes that are introduced either with our GUI tools will produce meaningful text diffs.
